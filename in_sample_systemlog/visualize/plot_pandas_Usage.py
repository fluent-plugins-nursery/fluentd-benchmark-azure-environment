...
parser.add_argument('--resource',
                    choices=['cpu_s', 'rss_s', 'vms_s', 'cpu_w',
                             'rss_w', 'vms_w',
...
message_repeated = pd.read_csv(os.path.join(base_path, 'usage-fluent-systemlog.tsv'), sep='\t', na_values='.')
message_ignore_repeated = pd.read_csv(os.path.join(base_path, 'usage-fluent-systemlog-ignore-repeated.tsv'), sep='\t', na_values='.')

df = pd.DataFrame({
    'message_repeated': message_repeated[resource_key],
    'message_ignore_repeated': message_ignore_repeated[resource_key],
})
if divide_base > 1:
    df = df.divide(divide_base)

medians = {
    'message_repeated': np.round(df['message_repeated'].median(), 2),
    'message_ignore_repeated': np.round(df['message_ignore_repeated'].median(), 2)}
...
data_range = ['message_repeated', 'message_ignore_repeated']
tick = 0
